    ...
    <section className="w-full py-5 overflow-hidden">
      <div className="w-full h-full overflow-y-auto grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-5 px-2">
        {imageList.map(({ url, state }, i) => (
          <div
            key={i}
            className={`w-full h-full min-h-28 relative p-3 rounded-md shadow-2xl shadow-primaryColor bg-primaryColor hover:bg-secondaryColor transitionCommon cursor-pointer border-4 ${
